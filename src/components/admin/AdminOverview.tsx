...
export const AdminOverview: React.FC<AdminOverviewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-4 max-w-full overflow-x-hidden animate-fade-in pb-4">
      ...
      {/* Academic Branches: Small Square-shaped Tiles 2 in a row on Mobile */}
      <div id="branch-analytics-section" className="bg-white p-3.5 sm:p-4 rounded-xl border border-[#DCE3ED] shadow-2xs space-y-3">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
          {departments.map((dept: any) => {
            const attPct = dept.overallAttendance ?? 0;
            const marksAvg = dept.overallTestMarkAvg ?? 0;
            ...
            return (
              <div
                ...
                className="p-3 rounded-xl border border-[#DCE3ED] bg-white hover:border-[#2E6FB0] transition-all flex flex-col justify-between aspect-square cursor-pointer shadow-2xs hover:shadow-xs active:scale-98 group"
                onClick={() => {
                  setSelectedBranchCode(dept.code);
                  setIsBranchHubOpen(true);
                }}
              >
                {/* Header: Dept Badge & Students Count */}
                <div className="flex items-center justify-between gap-1">
                  <span className="font-mono font-bold text-[11px] px-2 py-0.5 rounded bg-[#13284A] text-white shadow-2xs group-hover:bg-[#2E6FB0] transition-colors">
                    {dept.code}
                  </span>
                ...
                <div className="my-auto py-1">
                  <h3 className="text-xs font-bold text-[#13284A] line-clamp-2 leading-tight group-hover:text-[#2E6FB0] transition-colors">
                    {dept.name}
                  </h3>
                </div>

                {/* Compact Mini Metrics in Bottom */}
                <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-1 text-center">
                  <div className="bg-slate-50 rounded p-1 border border-slate-100">
                    <span className="text-[8px] font-bold text-slate-400 block uppercase">Att.</span>
                    <span className={`text-[11px] font-bold font-mono ${
                      attPct >= 80 ? 'text-emerald-700' : attPct > 0 ? 'text-amber-600' : 'text-slate-500'
                    }`}>
                      {attPct > 0 ? `${attPct}%` : '-'}
                    </span>
                  </div>
                  <div className="bg-slate-50 rounded p-1 border border-slate-100">
                    <span className="text-[8px] font-bold text-slate-400 block uppercase">Avg</span>
                    <span className="text-[11px] font-bold font-mono text-[#2E6FB0]">
                      {marksAvg > 0 ? marksAvg : '-'}
                    </span>
                  </div>
                </div>
