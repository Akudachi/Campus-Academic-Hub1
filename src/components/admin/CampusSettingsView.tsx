    ...
    <div className="space-y-3.5 max-w-full overflow-x-hidden animate-fade-in pb-4">
      ...
      {activeSubTab === 'backup' && (
        <div className="space-y-3 text-xs">
          ...
          <div className={`p-4 rounded-xl border shadow-2xs space-y-3 ${supabaseStatus?.connected ? 'bg-emerald-50/80 border-emerald-200' : 'bg-amber-50/80 border-amber-200'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-xs text-slate-900 flex items-center gap-1.5">
                    <Database className={`w-4 h-4 ${supabaseStatus?.connected ? 'text-emerald-700' : 'text-amber-700'}`} />
                    Supabase Cloud Database (Primary Source of Truth)
                  </h2>
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                    ...
                    {supabaseStatus?.connected ? `Connected (${supabaseStatus.provider})` : 'Cloud Database Unconnected'}
                  </span>
                </div>
                <p className="text-[11px] text-slate-700">
                  Host: <code className="font-mono bg-white/70 px-1 py-0.5 rounded border border-slate-200">{supabaseStatus?.databaseHost || 'Not configured'}</code>.
                  All academic records, student enrollments, faculty profiles, attendance logs, and CIE marks are permanently saved in Supabase and persist safely across Cloud Run scale-to-zero lifecycles.
                </p>
                {supabaseStatus?.error && (
                  <div className="mt-1 p-2 rounded-lg bg-rose-50 border border-rose-200 text-rose-800 text-[11px] space-y-0.5">
                    <div className="font-bold flex items-center gap-1">
                      <AlertTriangle className="w-3.5 h-3.5 text-rose-600" />
                      <span>Connection Error: {supabaseStatus.error}</span>
                    </div>
                    {supabaseStatus.diagnostic && (
                      <div className="text-[10px] text-rose-700 font-mono pl-4.5">
                        {supabaseStatus.diagnostic}
                      </div>
                    )}
                  </div>
                )}
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-slate-600 pt-0.5">
                  <span>Host: <strong className="font-mono">{supabaseStatus?.databaseHost}</strong></span>
                  <span>•</span>
                  <span>Last Cloud Sync: <strong>{supabaseStatus?.lastSyncTime ? new Date(supabaseStatus.lastSyncTime).toLocaleTimeString() : 'Active'}</strong></span>
                  <span>•</span>
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  disabled={cloudSyncing}
                  className="px-3 py-2 rounded-lg bg-white border border-slate-300 text-slate-800 font-bold hover:bg-slate-100 transition-colors flex items-center justify-center gap-1.5 shadow-2xs disabled:opacity-50"
